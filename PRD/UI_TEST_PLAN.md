...
| UI-016 | Todoist-deleted tasks disappear from sidebar | desktop | `todoist-overdue` | active | Verifies that tasks removed from Todoist (simulated orphan sweep) are hidden from the overdue/today pool on the next sync, while other Todoist tasks are unaffected and the deleted task surfaces in the trash dialog. |
