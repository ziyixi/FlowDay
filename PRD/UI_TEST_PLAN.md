...
| UI-050 | Quick-labelled tasks collapse into a quiet arrangeable Quick block | `quick-task-group` | desktop | Tasks labeled `quick` stay out of ordinary task cards, keep editable individual estimates, keep the aggregate Quick block visible when collapsed, and require selecting a real quick task before timer/Pomodoro starts. | `flow-and-shell.spec.ts` |
