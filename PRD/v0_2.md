...
- Display tasks grouped by: Today, Overdue, and by Project
- Each task card shows: title, priority color dot, project name, estimated duration
...
- Each task card shows: title, estimated duration, actual time (when timer runs), priority indicator
...
- Visual progress bar at the bottom: tasks completed vs. total, total estimated time vs. actual
- Completed tasks shown in a dimmed section with undo button
